...
class UMLSDataTransform(DataTransform):

    def __init__(self, path_to_MRCONSO_file, path_to_MRREL_file):
        self.path_to_MRCONSO_file= path_to_MRCONSO_file
        self.path_to_MRREL_file =path_to_MRREL_file

    def transform(self, limit=500000000000000000):
        with open(self.path_to_MRCONSO_file, "r") as f:
            mrconso_lines = f.readlines()

        atom_string_map = {}
        cuid_string_map = {}
        count=0
        for l in tqdm(mrconso_lines):
            cells = l.split("|")
            if cells[1] != 'ENG':
                continue
            cuid = cells[0]
            ...
            if count==limit:
                break
        print(f"Atom string map : {len(atom_string_map)}")
        ...
        MRREL_pd = pd.read_csv(self.path_to_MRREL_file, sep="|", header=None, nrows=limit)
        col_map = {}
        for idx, head in enumerate(headers):
            col_map[idx] = head
        MRREL_pd = MRREL_pd.rename(columns=col_map)

        MRREL_pd_Snomed = MRREL_pd[(MRREL_pd["SAB"] == "MSH") | (MRREL_pd["SAB"] == "SNOMEDCT_US")]  # Only use SNOMEDCT_US
        # MRREL_pd_Snomed = MRREL_pd
        MRREL_pd_Snomed_PAR_CHD = MRREL_pd_Snomed[
            (MRREL_pd_Snomed["REL"] == "PAR") | (MRREL_pd_Snomed["REL"] == "CHD")
            ]  # Only use PAR/CHD relations
        # MRREL_pd_Snomed_RO = MRREL_pd
        MRREL_pd_Snomed_PAR_CHD.loc[:, "STR1"] = MRREL_pd_Snomed_PAR_CHD["CUI1"].apply(
            lambda cuid: cuid_string_map[cuid] if (cuid in cuid_string_map) else None
        )
        MRREL_pd_Snomed_PAR_CHD.loc[:, "STR2"] = MRREL_pd_Snomed_PAR_CHD["CUI2"].apply(
            lambda cuid: cuid_string_map[cuid] if (cuid in cuid_string_map) else None
        )
        MRREL_pd_Snomed_PAR_CHD.loc[:, "RELA"] = MRREL_pd_Snomed_PAR_CHD["RELA"].apply(
            lambda rela: rela.replace("_", " ") if type(rela) == str else None
        )
        kg_pd_snomed_ro = MRREL_pd_Snomed_PAR_CHD[["STR1", "RELA", "STR2"]]
        print(f"Length of items: {len(kg_pd_snomed_ro.index)}")
        G = nx.MultiDiGraph()
        relation_list = []
        nodes =[]
        concept_names=[]
        for idx, item in tqdm(MRREL_pd_Snomed_PAR_CHD.iterrows()):
            if item['CUI1'] == item['CUI2']:
                continue
            if item['STR1'] == None or item['STR2'] == None:
                continue
            if not G.has_node(item['CUI1']):
                G.add_node(item['CUI1'], entity=item['STR1'], cui=item['CUI1'])
            if not G.has_node(item['CUI2']):
                G.add_node(item['CUI2'], entity=item['STR2'], cui=item['CUI2'])

            G.add_edge(item['CUI2'], item['CUI1'],
                       relation=item['RELA'])  ##### relation between CUI2 to CUI1: CUI2--> CUI1
            relation_list.append(item['RELA'])
            if item['CUI1'] not in nodes:
                nodes.append(item['CUI1'])
                concept_names.append(item['STR1'])

            if item['CUI2'] not in nodes:
                nodes.append(item['CUI2'])
                concept_names.append(item['STR2'])
        node_id = {n: i for i, n in enumerate(nodes)}
        edges = []
        ...
        return nodes, concept_names, edges
